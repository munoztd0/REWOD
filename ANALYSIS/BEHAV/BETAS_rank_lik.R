...
BETAS_R_N_CSp <- read.delim(file.path(analysis_path, taskHED,'ROI', paste('extracted_betas_',con_nameHED, '_via_', con_namePIT, '.txt',sep="")), header = T, sep ='\t') # read in dataset

LIK_R_N <- read.delim(file.path(analysis_path, taskHED, 'GLM-04', 'group_covariates', paste(conHED,'_', mod1, '_rank.txt',sep="")), header = T, sep ='\t') # read in dataset

INT_R_N <- read.delim(file.path(analysis_path, taskHED, 'GLM-04', 'group_covariates', paste(conHED,'_', mod2, '_rank.txt',sep="")), header = T, sep ='\t')  # read in dataset
...
BETAS_CSp_CSm_RN <- read.delim(file.path(analysis_path, taskPIT, 'ROI', paste('extracted_betas_',con_namePIT,'_via_', con_nameHED,'.txt',sep="")), header = T, sep ='\t') # read in dataset
...
A1 <- ggplot(R_N_df, aes(lik, AMY_BLA_LEFT_betas)) + #A2
  geom_point( size = 1) +
