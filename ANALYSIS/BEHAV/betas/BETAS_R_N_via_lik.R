...
BETAS_R_N <- read.delim(file.path(analysis_path, 'ROI', paste('extracted_betas_',con_name2, '_via_', conMOD, '.txt',sep="")), header = T, sep ='\t') # read in dataset
...
#R_N_df = merge(LIK_R_N_df, INT_R_N, by.x = "ID", by.y = "subj", all.x = TRUE)

# define factors
R_N_df$ID <- factor(R_N_df$ID)
#R_N_CSp$ID <- factor(R_N_CSp$ID)

R_N_df <- filter(R_N_df, ID != "24")
...
figure1 <- annotate_figure(figure1,
                           top = text_grob("Coeficient of determination: REWARD - NEUTRAL via lik for LIKING", color = "black", face = "bold", size = 14),
                           bottom = "Figure 1", fig.lab.face = "bold")

pdf('~/REWOD/DERIVATIVES/BEHAV/HED/R_N_via_lik_lik_coeff.pdf')
plot(figure1)
dev.off()
...
pdf('~/REWOD/DERIVATIVES/BEHAV/HED/R_N_viaR_lik_int_coeff.pdf')
plot(figure2)
dev.off()
# 
# 
