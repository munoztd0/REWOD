...
figure1 <- ggarrange(A1,B1,C1,D1,A2,B2,
                     labels = c( "CAUD_post_RIGHT_betas", "OFC2_RIGHT_betas",      "OFC_RIGHT_betas"  ,     "OPERCULUM_RIGHT_betas" ,
                     "PUT_ant_LEFT_betas" , "pINS_LEFT_betas"),
                     ncol = 2, nrow = 3,
                     vjust=3, hjust=0) 


figure1 <- annotate_figure(figure1,
                           top = text_grob("Coeficient of determination: REWARD - NEUTRAL via R_N_lik for LIKING", color = "black", face = "bold", size = 14),
                           bottom = "Figure 1", fig.lab.face = "bold")

pdf('~/REWOD/DERIVATIVES/BEHAV/HED/R_N_via_R_N_lik_lik_coeff.pdf')
plot(figure1)
dev.off()
