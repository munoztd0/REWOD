...
# open dataset 
#BETAS_CSp_CSm <- read.delim(file.path(analysis_path, 'ROI', paste('extracted_betas_',con_name2,'.txt',sep="")), header = T, sep ='\t') # read in dataset
BETAS_CSp_CSm <- read.delim(file.path(analysis_path, 'ROI', paste('extracted_betas_',con_name2, '_via_', conMOD, '.txt',sep="")), header = T, sep ='\t') # read in dataset


EFF_CSp_CSm <- read.delim(file.path(analysis_path, 'GLM-04', 'group_covariates', paste(con2,'_', mod1, '_rank.txt',sep="")), header = T, sep ='\t') # read in dataset

#INT_CSp_CSm <- read.delim(file.path(analysis_path, 'GLM-04', 'group_covariates', paste(con2,'_', mod2, '_meancent.txt',sep="")), header = T, sep ='\t')  # read in dataset


# merge
#CSp_CSm_CSp = merge(BETAS_CSp_CSm_CSp, EFF_CSp_CSm, by.x = "ID", by.y = "subj", all.x = TRUE)
#CSp_CSm_CSp = merge(CSp_CSm_CSp, INT_CSp_CSm, by.x = "ID", by.y = "subj", all.x = TRUE)

CSp_CSm_df = merge(BETAS_CSp_CSm, EFF_CSp_CSm, by.x = "ID", by.y = "subj", all.x = TRUE)
#CSp_CSm_df = merge(CSp_CSm_df, INT_CSp_CSm, by.x = "ID", by.y = "subj", all.x = TRUE)

#CSp_CSm_df = merge(EFF_CSp_CSm_df, INT_CSp_CSm, by.x = "ID", by.y = "subj", all.x = TRUE)

# define factors
CSp_CSm_df$ID <- factor(CSp_CSm_df$ID)
#CSp_CSm_CSp$ID <- factor(CSp_CSm_CSp$ID)

CSp_CSm_df <- filter(CSp_CSm_df, ID != "24")
...
# A2 <- ggplotRegression(lm(CSp_CSm_df[[6]]~eff)) + rremove("x.title")
# B2 <- ggplotRegression(lm(CSp_CSm_df[[7]]~eff)) + rremove("x.title")
# C2 <- ggplotRegression(lm(CSp_CSm_df[[8]]~eff)) + rremove("x.title")
# D2 <- ggplotRegression(lm(CSp_CSm_df[[9]]~eff)) + rremove("x.title")
# A3 <- ggplotRegression(lm(CSp_CSm_df[[10]]~eff)) + rremove("x.title")
# B3 <- ggplotRegression(lm(CSp_CSm_df[[11]]~eff)) + rremove("x.title")
# C3 <- ggplotRegression(lm(CSp_CSm_df[[12]]~eff)) + rremove("x.title")
# D3 <- ggplotRegression(lm(CSp_CSm_df[[13]]~eff)) + rremove("x.title")
# A4 <- ggplotRegression(lm(CSp_CSm_df[[14]]~eff)) + rremove("x.title")
# B4 <- ggplotRegression(lm(CSp_CSm_df[[15]]~eff)) + rremove("x.title")
# C4 <- ggplotRegression(lm(CSp_CSm_df[[16]]~eff)) + rremove("x.title")
# D4 <- ggplotRegression(lm(CSp_CSm_df[[17]]~eff)) + rremove("x.title")
# A5 <- ggplotRegression(lm(CSp_CSm_df[[18]]~eff)) + rremove("x.title")
# B5 <- ggplotRegression(lm(CSp_CSm_df[[19]]~eff)) + rremove("x.title")
# 
# 
# 
# 
# 
# figure1 <- ggarrange(A1,B1,C1,D1,A2,B2,C2,D2,A3,B3,C3,D3,A4,B4,C4,D4,A5,B5,
#                      labels = c( "CAUD_ANT_RIGHT_betas" ,  "CAUD_VENTR_LEFT_betas" , "CAUD_VENTR_RIGHT_betas", "CLOSTRUM_LEFT_betas"  ,  "FRONTAL_LEFT_betas",    
#  "FRONTAL_RIGHT_betas" ,   "NACC_RIGHT_betas"   ,    "OFC_LEFT_betas"   ,      "OFC_RIGHT_betas" ,       "SUBCAL_LEFT_betas" ,     "SUBCAL_RIGHT_betas",    
#  "aINS_RIGHT_betas"  ,     "dlPFC_LEFT_betas"  ,     "dlPFC_RIGHT_betas"   ,   "pINS_LEFT_betas"  ,      "pINS_RIGHT_betas" ,      "vmPFC_LEFT_betas",      
#                                  "vmPFC_RIGHT_betas"),
#                      ncol = 2, nrow = 9,
#                      vjust=3, hjust=0) 
# 
...
#                            top = text_grob("Coeficient of determination: CSp-CSm via eff for Effort", color = "black", face = "bold", size = 14),
#                            bottom = "Figure 1", fig.lab.face = "bold")
# 
# pdf('~/REWOD/DERIVATIVES/BEHAV/PIT/CSp_CSm_via_eff_eff_coeff.pdf')
# plot(figure1)
# dev.off()
...
A1 <- ggplotRegression(lm(CSp_CSm_df[[3]]~eff)) + rremove("x.title")
B1 <- ggplotRegression(lm(CSp_CSm_df[[4]]~eff)) + rremove("x.title")
C1 <- ggplotRegression(lm(CSp_CSm_df[[5]]~eff)) + rremove("x.title")
D1 <- ggplotRegression(lm(CSp_CSm_df[[8]]~eff)) + rremove("x.title")
A2 <- ggplotRegression(lm(CSp_CSm_df[[11]]~eff)) + rremove("x.title")
B2 <- ggplotRegression(lm(CSp_CSm_df[[12]]~eff)) + rremove("x.title")
C2 <- ggplotRegression(lm(CSp_CSm_df[[13]]~eff)) + rremove("x.title")
D2 <- ggplotRegression(lm(CSp_CSm_df[[18]]~eff)) + rremove("x.title")
...
figure2 <- ggarrange(A1,B1,C1,D1,A2,B2,C2,D2,
                     labels = c( "CAUD_VENTR_LEFT_betas" , "CAUD_VENTR_RIGHT_betas", "CLOSTRUM_LEFT_betas"  ,   "NACC_RIGHT_betas"   ,   
                                 "SUBCAL_LEFT_betas"  ,    "SUBCAL_RIGHT_betas", "aINS_RIGHT_betas"  ,  "vmPFC_LEFT_betas"),
                     ncol = 2, nrow = 4,
                     vjust=3, hjust=0) 
...
int = CSp_CSm_df$int
int = zscore(int)
A1 <- ggplotRegression(lm(CSp_CSm_df[[2]]~int)) + rremove("x.title")
B1 <- ggplotRegression(lm(CSp_CSm_df[[3]]~int)) + rremove("x.title")
C1 <- ggplotRegression(lm(CSp_CSm_df[[4]]~int)) + rremove("x.title")
D1 <- ggplotRegression(lm(CSp_CSm_df[[5]]~int)) + rremove("x.title")
A2 <- ggplotRegression(lm(CSp_CSm_df[[6]]~int)) + rremove("x.title")
B2 <- ggplotRegression(lm(CSp_CSm_df[[7]]~int)) + rremove("x.title")
C2 <- ggplotRegression(lm(CSp_CSm_df[[8]]~int)) + rremove("x.title")
D2 <- ggplotRegression(lm(CSp_CSm_df[[9]]~int)) + rremove("x.title")
A3 <- ggplotRegression(lm(CSp_CSm_df[[10]]~int)) + rremove("x.title")
B3 <- ggplotRegression(lm(CSp_CSm_df[[11]]~int)) + rremove("x.title")
C3 <- ggplotRegression(lm(CSp_CSm_df[[12]]~int)) + rremove("x.title")
D3 <- ggplotRegression(lm(CSp_CSm_df[[13]]~int)) + rremove("x.title")




figure2 <- ggarrange(A1,B1,C1,D1,A2,B2,C2,D2,A3,B3,C3,D3,
                     labels = c( "CAUD_VENTR_LEFT_betas",  "CAUD_VENTR_RIGHT_betas" ,"NACC_RIGHT_betas"  ,     "NAcc_Left_Clu_betas"  , 
                                 "OFC_LEFT_betas"     ,    "OFC_RIGHT_betas"   ,     "PUT_POST_RIGHT_betas"  , "Put_ant_LEFT_betas" ,    "SUBCAL_LEFT_betas"   ,  
                                 "SUBCAL_RIGHT_betas"   ,  "aINS_LEFT_betas" ,       "aINS_RIGHT_betas"),
                     ncol = 2, nrow = 6,
                     vjust=3, hjust=0) 

figure2 <- annotate_figure(figure2,
                           top = text_grob("Coeficient of determination: REWARD - NO REWARD via lik  for INTENSITY", color = "black", face = "bold", size = 14),
                           bottom = "Figure 2", fig.lab.face = "bold")


pdf('~/REWOD/DERIVATIVES/BEHAV/HED/CSp_CSm_viaR_lik_int_coeff.pdf')
plot(figure2)
dev.off()
# 
# 
...
#                            top = text_grob("Coeficient of determination: REWARD - NO REWARD from CSp-CSm ROIs for LIKING", color = "black", face = "bold", size = 14),
#                            bottom = "Figure 1", fig.lab.face = "bold")
# 
...
#                            top = text_grob("Coeficient of determination: REWARD - NO REWARD from CSp-CSm ROIs for INTENSITY", color = "black", face = "bold", size = 14),
#                            bottom = "Figure 1", fig.lab.face = "bold")
# 
