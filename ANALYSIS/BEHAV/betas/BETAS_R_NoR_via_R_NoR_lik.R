...
BETAS_R_NoR <- read.delim(file.path(analysis_path, 'ROI', paste('extracted_betas_',con_name2, '_via_', conMOD, '.txt',sep="")), header = T, sep ='\t') # read in dataset
...
INT_R_NoR <- read.delim(file.path(analysis_path, 'GLM-04', 'group_covariates', paste(con2,'_', mod2, '_meancent.txt',sep="")), header = T, sep ='\t')  # read in dataset
...
#R_NoR_df = merge(LIK_R_NoR_df, INT_R_NoR, by.x = "ID", by.y = "subj", all.x = TRUE)
...
B1 <- ggplotRegression(lm(R_NoR_df[[3]]~lik)) + rremove("x.title")
D1 <- ggplotRegression(lm(R_NoR_df[[5]]~lik)) + rremove("x.title")
A2 <- ggplotRegression(lm(R_NoR_df[[6]]~lik)) + rremove("x.title")
B2 <- ggplotRegression(lm(R_NoR_df[[7]]~lik)) + rremove("x.title")
C2 <- ggplotRegression(lm(R_NoR_df[[8]]~lik)) + rremove("x.title")
D2 <- ggplotRegression(lm(R_NoR_df[[9]]~lik)) + rremove("x.title")
A3 <- ggplotRegression(lm(R_NoR_df[[10]]~lik)) + rremove("x.title")
B3 <- ggplotRegression(lm(R_NoR_df[[11]]~lik)) + rremove("x.title")
C3 <- ggplotRegression(lm(R_NoR_df[[12]]~lik)) + rremove("x.title")
D3 <- ggplotRegression(lm(R_NoR_df[[13]]~lik)) + rremove("x.title")
A4 <- ggplotRegression(lm(R_NoR_df[[14]]~lik)) + rremove("x.title")
B4 <- ggplotRegression(lm(R_NoR_df[[15]]~lik)) + rremove("x.title")
C4 <- ggplotRegression(lm(R_NoR_df[[16]]~lik)) + rremove("x.title")





figure1 <- ggarrange(A1,B1,D1,A2,B2,C2,D2,A3,B3,C3,D3,A4,B4,C4,
                     labels = c(  "CAUD_LEFT_betas"  ,   "Caud_Left_Clu_betas", "IFG_LEFT_betas" ,     "IFG_RIGHT_betas" ,
                                  "OFC_RIGHT_betas",    "OPER_LEFT_betas"   ,  "PUT_LEFT_betas"    ,  "PUT_ant_LEFT_betas" , "Put_Left_clu_betas" ,
                                  "SUBCAL_RIGHT_betas" , "midbrain_betas" , "vlPFC_RIGHT_betas",    "vmPFC_LEFT_betas" ,   "vmPFC_RIGHT_betas"),
                     ncol = 2, nrow = 7,
                     vjust=3, hjust=0) 


figure1 <- annotate_figure(figure1,
                           top = text_grob("Coeficient of determination: REWARD - NO REWARD via R_NoR_lik for LIKING", color = "black", face = "bold", size = 14),
                           bottom = "Figure 1", fig.lab.face = "bold")

pdf('~/REWOD/DERIVATIVES/BEHAV/HED/R_NoR_via_R_NoR_lik_lik_coeff.pdf')
plot(figure1)
dev.off()
...
int = R_NoR_df$int
int = zscore(int)
A1 <- ggplotRegression(lm(R_NoR_df[[2]]~int)) + rremove("x.title")
B1 <- ggplotRegression(lm(R_NoR_df[[3]]~int)) + rremove("x.title")
C1 <- ggplotRegression(lm(R_NoR_df[[4]]~int)) + rremove("x.title")
D1 <- ggplotRegression(lm(R_NoR_df[[5]]~int)) + rremove("x.title")
A2 <- ggplotRegression(lm(R_NoR_df[[6]]~int)) + rremove("x.title")
B2 <- ggplotRegression(lm(R_NoR_df[[7]]~int)) + rremove("x.title")
C2 <- ggplotRegression(lm(R_NoR_df[[8]]~int)) + rremove("x.title")
D2 <- ggplotRegression(lm(R_NoR_df[[9]]~int)) + rremove("x.title")
A3 <- ggplotRegression(lm(R_NoR_df[[10]]~int)) + rremove("x.title")
B3 <- ggplotRegression(lm(R_NoR_df[[11]]~int)) + rremove("x.title")
C3 <- ggplotRegression(lm(R_NoR_df[[12]]~int)) + rremove("x.title")
D3 <- ggplotRegression(lm(R_NoR_df[[13]]~int)) + rremove("x.title")
...
pdf('~/REWOD/DERIVATIVES/BEHAV/HED/R_NoR_viaR_lik_int_coeff.pdf')
plot(figure2)
dev.off()
# 
# 
