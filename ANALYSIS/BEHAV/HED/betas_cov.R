...
analysis_pathEFF <- file.path('~/REWOD/DERIVATIVES/ANALYSIS', taskEFF) 
setwd(analysis_path)

# open dataset 
#BETAS_R_N <- read.delim(file.path(analysis_path, 'ROI', paste('extracted_betas_',con_name1,'_', mod1, '.txt',sep="")), header = T, sep ='\t') # read in dataset
BETAS_R_N <- read.delim(file.path(analysis_path, 'ROI', paste('extracted_betas_',con_name2,'_', mod1, '.txt',sep="")), header = T, sep ='\t') # read in dataset
...
# R_C_df = merge(R_C_df, EFF, by.x = "ID", by.y = "subj", all.x = TRUE)

R_N_lik_df = merge(BETAS_R_N, LIK_R_N, by.x = "ID", by.y = "subj", all.x = TRUE)
#R_N_df = merge(R_N_df, INT_R_N, by.x = "ID", by.y = "subj", all.x = TRUE)
R_N_lik_df = merge(R_N_lik_df, EFF, by.x = "ID", by.y = "subj", all.x = TRUE)


# define factors
#R_C_df$ID <- factor(R_C_df$ID)
R_N_lik_df$ID <- factor(R_N_df$ID)
...
lik = R_N_lik_df$lik
A <- ggplotRegression(lm(R_N_lik_df[[2]]~lik)) + rremove("x.title")
B <- ggplotRegression(lm(R_N_lik_df[[3]]~lik)) + rremove("x.title")
C <- ggplotRegression(lm(R_N_lik_df[[4]]~lik)) + rremove("x.title")
D <- ggplotRegression(lm(R_N_lik_df[[5]]~lik)) + rremove("x.title")


figure2 <- ggarrange(A,B,C,D,
                     labels = c("CAUD_post_R", "OFC2_R",      "OFC_R" ,      "OPERCULUM_R"),
                     ncol = 2, nrow = 2,
                     vjust=3, hjust=0) 

figure2 <- annotate_figure(figure2,
                top = text_grob("Coeficient of determination: REWARD-NEUTRAL lik for liking", color = "black", face = "bold", size = 14),
                bottom = "Figure 2", fig.lab.face = "bold")

pdf('~/REWOD/DERIVATIVES/BEHAV/HED/R_N_lik_lik_coeff.pdf')
plot(figure2)
dev.off()
...
eff = R_N_lik_df$eff
A1 <- ggplotRegression(lm(R_N_lik_df[[2]]~eff)) + rremove("x.title")
B1 <- ggplotRegression(lm(R_N_lik_df[[3]]~eff)) + rremove("x.title")
C1 <- ggplotRegression(lm(R_N_lik_df[[4]]~eff)) + rremove("x.title")
D1 <- ggplotRegression(lm(R_N_lik_df[[5]]~eff)) + rremove("x.title")
...
figure3 <- annotate_figure(figure3,
                           top = text_grob("Coeficient of determination: REWARD-NEUTRAL lik for effort", color = "black", face = "bold", size = 14),
                           bottom = "Figure 3", fig.lab.face = "bold")

pdf('~/REWOD/DERIVATIVES/BEHAV/HED/R_N_lik_eff_coeff.pdf')
plot(figure3)
dev.off()#  Plot for R_NoR  ----------------------------------------------------------
...
# CORRELATIONS R_N_lik ------------------------------------------------------

corr_R_N.rcorr = rcorr(as.matrix(R_N_lik_df))
corr_R_N.coeff = corr_R_N.rcorr$r[2:5,8:9]
corr_R_N.p = corr_R_N.rcorr$P[2:5,8:9]

col3 <- colorRampPalette(c("blue", "white", "red"))
# PLOT CORR

pdf('~/REWOD/DERIVATIVES/BEHAV/HED/R_N_lik_corrplot.pdf')
