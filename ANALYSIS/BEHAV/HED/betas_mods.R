...
BETAS_R_N <- read.delim(file.path(analysis_path, 'ROI', paste('extracted_betas_', mod1,'.txt',sep="")), header = T, sep ='\t') # read in dataset
...
lik = R_N_df$lik
A <- ggplotRegression(lm(R_N_df[[2]]~lik)) + rremove("x.title")
B <- ggplotRegression(lm(R_N_df[[3]]~lik)) + rremove("x.title")
C <- ggplotRegression(lm(R_N_df[[4]]~lik)) + rremove("x.title")
...
A2 <- ggplotRegression(lm(R_N_df[[10]]~lik)) + rremove("x.title")
B2 <- ggplotRegression(lm(R_N_df[[11]]~lik))+ rremove("x.title")
C2 <- ggplotRegression(lm(R_N_df[[12]]~lik)) + rremove("x.title")
D2 <- ggplotRegression(lm(R_N_df[[13]]~lik)) + rremove("x.title")
...
corr_R_N.coeff = corr_R_N.rcorr$r[2:13,14]
corr_R_N.p = corr_R_N.rcorr$P[2:13,14]

col3 <- colorRampPalette(c("blue", "white", "red")) 
# PLOT CORR

pdf('~/REWOD/DERIVATIVES/BEHAV/HED/lik_corrplot.pdf')
