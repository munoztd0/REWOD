...
#BETAS_R_NoR_CSp <- read.delim(file.path(analysis_path, 'ROI', paste('extracted_betas_',con_name2, '_via_', conEFF, '.txt',sep="")), header = T, sep ='\t') # read in dataset
...
C1 <- ggplotRegression(lm(R_NoR_df[[4]]~lik)) + rremove("x.title")
...
figure1 <- ggarrange(A1,B1,C1,D1,A2,B2,C2,D2,
                     labels = c(  "AMY_BLA_LEFT_betas",  "AMY_BM_LEFT_betas" ,  "AMY_CMN_LEFT_betas",  "AMY_LA_LEFT_betas"  , "AMY_full_LEFT_betas",   "SUBCAL_LEFT_betas" ,  "SUBCAL_RIGHT_betas" , "VS_LEFT_betas"),
                     ncol = 2, nrow = 4,
                     vjust=3, hjust=0) 
...
pdf('~/REWOD/DERIVATIVES/BEHAV/HED/Reward_No_Reward_lik_coeff.pdf')
plot(figure1)
dev.off()
...
figure2 <- ggarrange(A1,B1,C1,D1,A2,B2,C2,D2,
                     labels = c(  "AMY_BLA_LEFT_betas",  "AMY_BM_LEFT_betas" ,  "AMY_CMN_LEFT_betas",  "AMY_LA_LEFT_betas"  , "AMY_full_LEFT_betas",
                                  "SUBCAL_LEFT_betas" ,  "SUBCAL_RIGHT_betas" , "VS_LEFT_betas"  ),
                     ncol = 2, nrow = 4,
                     vjust=3, hjust=0) 


figure2 <- annotate_figure(figure2,
                           top = text_grob("Coeficient of determination: Reward - No Reward  for INTENSITY", color = "black", face = "bold", size = 14),
                           bottom = "Figure 2", fig.lab.face = "bold")


pdf('~/REWOD/DERIVATIVES/BEHAV/HED/Reward_No_Reward_int_coeff.pdf')
plot(figure2)
dev.off()
...
# A2 <- ggplotRegression(lm(R_N_df[[6]]~lik)) + rremove("x.title")
# B2 <- ggplotRegression(lm(R_N_df[[7]]~lik)) + rremove("x.title")
# C2 <- ggplotRegression(lm(R_N_df[[8]]~lik)) + rremove("x.title")
# D2 <- ggplotRegression(lm(R_N_df[[9]]~lik)) + rremove("x.title")
# A3 <- ggplotRegression(lm(R_N_df[[10]]~lik)) + rremove("x.title")
# B3 <- ggplotRegression(lm(R_N_df[[11]]~lik)) + rremove("x.title")
# C3 <- ggplotRegression(lm(R_N_df[[12]]~lik)) + rremove("x.title")
# D3 <- ggplotRegression(lm(R_N_df[[13]]~lik)) + rremove("x.title")
# 
# 
# figure1 <- ggarrange(A1,B1,C1,D1,A2,B2,C2,D2,A3,B3,C3,D3,
#                      labels = c("CAUD_LEFT_betas" ,   "IFG_LEFT_betas"  ,   "IFG_RIGHT_betas"  ,  "OFC_RIGHT_betas"   , "OPER_LEFT_betas" ,  
#                                 "PUT_LEFT_betas" ,    "SUBCAL_RIGHT_betas", "Snr_betas"   ,       "midbrain_betas" ,    "vlPFC_RIGHT_betas" , "vmPFC_LEFT_betas"  ,
#                                 "vmPFC_RIGHT_betas"),
#                      ncol = 2, nrow = 6,
#                      vjust=3, hjust=0) 
# 
...
# A2 <- ggplotRegression(lm(R_N_df[[6]]~int)) + rremove("x.title")
# B2 <- ggplotRegression(lm(R_N_df[[7]]~int)) + rremove("x.title")
# C2 <- ggplotRegression(lm(R_N_df[[8]]~int)) + rremove("x.title")
# D2 <- ggplotRegression(lm(R_N_df[[9]]~int)) + rremove("x.title")
# A3 <- ggplotRegression(lm(R_N_df[[10]]~int)) + rremove("x.title")
# B3 <- ggplotRegression(lm(R_N_df[[11]]~int)) + rremove("x.title")
# C3 <- ggplotRegression(lm(R_N_df[[12]]~int)) + rremove("x.title")
# D3 <- ggplotRegression(lm(R_N_df[[13]]~int)) + rremove("x.title")
# 
# 
