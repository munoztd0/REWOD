...
PIT.bct = ddply(REWOD_PIT, .(trial, condition), summarise,  n_grips = mean(n_grips, na.rm = TRUE))
...
PIT.bsc = ddply(REWOD_PIT, .(id, condition), summarise,  n_grips = mean(n_grips, na.rm = TRUE))
PIT.bsc.base = PIT.bsc %>% filter(condition == 'Baseline')
PIT.bsc.CSp = PIT.bsc %>% filter(condition == 'CSplus')
PIT.bsc.CSm = PIT.bsc %>% filter(condition == 'CSminus')


ggplot(PIT.bsc.base, aes(as.factor(id), n_grips)) +
  geom_col(fill = 'black') +
  theme_bw() +
  ...
  labs(
    title = "Baseline by subject",
    x = "Subject",
    y = "Mean number of grips"
  )

ggplot(PIT.bsc.CSp, aes(as.factor(id), n_grips)) +
  geom_col(fill = 'blue') +
  theme_bw() +
  scale_y_continuous(limits=c(0,40)) +
  theme(plot.title = element_text(hjust = 0.5, size=20,  face="bold"), axis.title = element_text(size=14), axis.text=element_text(size=14), legend.position = c(0.9, 0.9), legend.title=element_blank()) +
  labs(
    title = "CSp by subject",
    x = "Subject",
    y = "Mean number of grips"
  )

ggplot(PIT.bsc.CSm, aes(as.factor(id), n_grips), color = I('red')) +
  geom_col(fill = 'red') +
  theme_bw() +
  scale_y_continuous(limits=c(0,40)) +
  theme(plot.title = element_text(hjust = 0.5, size=20,  face="bold"), axis.title = element_text(size=14), axis.text=element_text(size=14), legend.position = c(0.9, 0.9), legend.title=element_blank()) +
  labs(
    title = "CSm by subject",
    x = "Subject",
    y = "Mean number of grips"
  )

# ##plot n_grips to see by conditions
ggplot(PIT.bsc , aes(reorder(id, -n_grips), y = n_grips, color=condition)) +
  geom_point() +
  ...
  theme(plot.title = element_text(hjust = 0.5, size=20,  face="bold"), axis.title = element_text(size=14), axis.text=element_text(size=14), legend.position = c(0.9, 0.9), legend.title=element_blank()) +
  labs(
    title = "n_grips by subject by condition",
    x = "Subject",
    y = "Mean number of grips reordered"
  )

plot(density(PIT.bsc.CSp$n_grips));
plot(density(PIT.bsc.CSm$n_grips));
plot(density(PIT.bsc.base$n_grips));
...
dfbetas(alt.est.id)  #(standardized difference of the beta) “too much” is defined by the 2/sqrt(n) = 0.408 OR 1 
